...
Matrix q_integration_matrix(const size_t number_of_points, const double coeff) {
  ...
  lhs_mat(0, 1) += -coeff;
  lhs_mat(number_of_points - 1, number_of_points - 2) += coeff;
...
template <>
void radial_integrate_cce_pole_equations<1>(
    const gsl::not_null<ComplexDataVector*> integral_result,
    const ComplexDataVector& pole_of_integrand,
    const ComplexDataVector& regular_integrand,
    const ComplexDataVector& boundary, const ComplexDataVector& one_minus_y,
  ...
  const ComplexDataVector one_minus_y_radial =
      1.0 -
      std::complex<double>(1.0, 0.0) *
          Spectral::collocation_points<Spectral::Basis::Legendre,
                                       Spectral::Quadrature::GaussLobatto>(
              number_of_radial_points);
  *integral_result += outer_product(boundary_correction, one_minus_y_radial);
}
...
void RadialIntegrateBondi<BoundaryPrefix, Tags::BondiQ>::apply(
    ...
    const size_t l_max, const size_t number_of_radial_points) {
  radial_integrate_cce_pole_equations<2>(
