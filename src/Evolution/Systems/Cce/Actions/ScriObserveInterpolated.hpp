...
auto one_minus_y_inter = db::get<Tags::OneMinusY>(box);
    std::vector<std::string> file_legendnewnew;
    file_legendnewnew.reserve(number_of_radial_points + 1);
    file_legendnewnew.emplace_back("time");
    for (size_t i = 0; i < number_of_radial_points; ++i) {
        file_legendnewnew.push_back(MakeString{} << "Real R_" << i);
    }
    std::vector<double> data_to_writenewnew(number_of_radial_points + 1);
              ScriObserveInterpolated::transform_and_write_y<
                  Tags::OneMinusY, 0, ParallelComponent>(
                  get(one_minus_y_inter),
                  interpolation_time,
                  make_not_null(&data_to_writenewnew), file_legendnewnew, l_max,
                  cache);
}
    }
 ...
 private:
  ...
  template <typename Tag, int Spin, typename ParallelComponent,
            typename Metavariables>
  static void transform_and_write_y(
      const SpinWeighted<ComplexDataVector, Spin>& data, const double time,
      const gsl::not_null<std::vector<double>*> data_to_write_buffer,
      const std::vector<std::string>& legend, const size_t l_max,
      Parallel::GlobalCache<Metavariables>& cache) noexcept {
    ...
    for (size_t i = 0; i < number_of_radial_points; ++i) {
      make_const_view(make_not_null(&to_transform.data()), data.data(),
                      i * number_of_angular_points, number_of_angular_points);
      (*data_to_write_buffer)[i + 1] = real(to_transform.data()[0]);
    }
