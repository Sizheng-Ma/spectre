...
struct STScriObserveInterpolated {
  using const_global_cache_tags =
      tmpl::flatten<tmpl::list<Tags::ObservationLMax>>;
  template <typename DbTags, typename... InboxTags, typename Metavariables,
            typename ArrayIndex, typename ActionList,
            typename ParallelComponent>
  static Parallel::iterable_action_return_t apply(
    ...
    while (db::get<Tags::InterpolationManager<
               ComplexDataVector, Tags::ScriPlus<Tags::BondiSTPsi>>>(box)
               .first_time_is_ready_to_interpolate()) {
      std::pair<double, ComplexDataVector> interpolation;
      db::mutate<Tags::InterpolationManager<ComplexDataVector,
                                            Tags::ScriPlus<Tags::BondiSTPsi>>>(
          [&interpolation](
              const gsl::not_null<ScriPlusInterpolationManager<
                  ComplexDataVector, Tags::ScriPlus<Tags::BondiSTPsi>>*>
                  interpolation_manager) {
            interpolation =
                interpolation_manager->interpolate_and_pop_first_time();
          },
          make_not_null(&box));
      STScriObserveInterpolated::transform_and_write<
          Tags::ScriPlus<Tags::BondiSTPsi>, 0, ParallelComponent>(
          interpolation.second, interpolation.first,
          make_not_null(&goldberg_modes), make_not_null(&data_to_write),
          file_legend, l_max, observation_l_max, cache);
    }
...
template <typename ObserverWriterComponent>
struct STAnaScriObserveInterpolated {
  using const_global_cache_tags =
      tmpl::flatten<tmpl::list<Tags::ObservationLMax>>;
  template <typename DbTags, typename... InboxTags, typename Metavariables,
            typename ArrayIndex, typename ActionList,
  ...
  static Parallel::iterable_action_return_t apply(
    ...
    while (
        db::get<Tags::InterpolationManager<ComplexDataVector,
                                           Tags::ScriPlus<Tags::BondiSTX>>>(box)
            .first_time_is_ready_to_interpolate()) {
      std::pair<double, ComplexDataVector> interpolation;
      db::mutate<Tags::InterpolationManager<ComplexDataVector,
          ...
          [&interpolation](
              const gsl::not_null<ScriPlusInterpolationManager<
                  ComplexDataVector, Tags::ScriPlus<Tags::BondiSTX>>*>
                  interpolation_manager) {
            interpolation =
                interpolation_manager->interpolate_and_pop_first_time();
          },
          make_not_null(&box));
      STAnaScriObserveInterpolated::transform_and_write<
          Tags::ScriPlus<Tags::BondiSTX>, 0, ParallelComponent>(
          interpolation.second, interpolation.first,
          make_not_null(&goldberg_modes), make_not_null(&data_to_write),
          file_legend, l_max, observation_l_max, cache);
    }
