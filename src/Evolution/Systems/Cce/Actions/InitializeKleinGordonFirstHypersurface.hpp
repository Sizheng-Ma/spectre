 ...
 * in the `Evolve` phase.
 */
template <typename BoundaryComponent>
struct InitializeKleinGordonFirstHypersurface {
  ...
  static Parallel::iterable_action_return_t apply(
    ...
    if constexpr (tt::is_a_v<KleinGordonAnalyticWorldtubeBoundary,
                             BoundaryComponent>) {
      db::mutate<Tags::KleinGordonPsi>(
          [](const gsl::not_null<Scalar<SpinWeighted<ComplexDataVector, 0>>*>
            ...
            for (size_t i = 0; i < number_of_radial_points; i++) {
              ...
              auto u0new = real(-get(bondi_r).data() -
                                4 * log(get(bondi_r).data() / 2. - 1.));
              auto one_over_r =
