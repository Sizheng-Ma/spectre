...
class KleinGordonAnalyticBoundaryDataManager {
 public:
  // charm needs an empty constructor.
  KleinGordonAnalyticBoundaryDataManager() = default;

  KleinGordonAnalyticBoundaryDataManager(
      size_t l_max, double extraction_radius,
  ...
  const Solutions::KleinGordonWorldtubeData& get_generator() const {
    return *generator_;
  }
 ...
 private:
  size_t l_max_ = 0;
  std::unique_ptr<Solutions::KleinGordonWorldtubeData> generator_;
  double extraction_radius_ = std::numeric_limits<double>::signaling_NaN();
};
}  // namespace Cce
