...
struct GaugeWave : public SphericalMetricData {
  ...
  struct PeakTime {
    ...
    static type lower_bound() { return -1000000000.0; }
  };
  struct Duration {
    using type = double;
    static constexpr Options::String help{
  ...
  using options = tmpl::list<Mass, Frequency, Amplitude, PeakTime, Duration>;
  ...
  GaugeWave(double mass, double frequency, double amplitude, double peak_time,
            double duration);
 ...
 private:
  double coordinate_wave_function(double radius) const;

  double du_coordinate_wave_function(double radius) const;

  double du_du_coordinate_wave_function(double radius) const;
