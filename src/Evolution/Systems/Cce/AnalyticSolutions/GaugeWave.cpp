...
GaugeWave::GaugeWave(const double mass, const double frequency,
                     const double amplitude, const double peak_time,
...
double GaugeWave::coordinate_wave_function(const double radius) const {
  const auto retarded_time = -radius;
  return amplitude_ * sin(frequency_ * retarded_time) *
         exp(-square(retarded_time - peak_time_) / square(duration_));
}

double GaugeWave::du_coordinate_wave_function(const double radius) const {
  const auto retarded_time = -radius;
  return amplitude_ *
...
double GaugeWave::du_du_coordinate_wave_function(const double radius) const {
  const auto retarded_time = -radius;
  return amplitude_ *
...
void GaugeWave::spherical_metric(
    const gsl::not_null<
        ...
        spherical_metric,
    const size_t /*l_max*/, const double ccetime) const {
  const double extraction_radius_ = -ccetime;

  const auto wave_f = coordinate_wave_function(extraction_radius_);
  const auto du_wave_f = du_coordinate_wave_function(extraction_radius_);
...
void GaugeWave::dr_spherical_metric(
  ...
  // t - r)
  dt_spherical_metric(dr_spherical_metric, l_max, ccetime);
...
void GaugeWave::dt_spherical_metric(
  ...
  const auto wave_f = coordinate_wave_function(extraction_radius_);
  const auto du_wave_f = du_coordinate_wave_function(extraction_radius_);
  const auto du_du_wave_f = du_du_coordinate_wave_function(extraction_radius_);
