...
void InterpolatePsi0<Dim, Frame>::interpolate_psi0(
  ...
  theta = atan2(
      sqrt(square(inertial_coords.get(0)) + square(inertial_coords.get(1))),
...
void AngularTetradForCCM<Dim, Frame>::get_m_vector(
  ...
  (*m).get(2) =
      std::complex<double>(1.0, 0.0) * cos(theta) * sin(phi) / sqrt(2.0);
  (*m).get(2) +=
      std::complex<double>(0.0, 1.0) * cos(phi) * sin(theta) / sqrt(2.0);
  (*m).get(1) =
      std::complex<double>(1.0, 0.0) * cos(theta) * cos(phi) / sqrt(2.0);
  (*m).get(1) -=
...
void IncomingWFromCCE<Dim, Frame>::wijfromcce(
  ...
  for (size_t a = 0; a <= Dim; ++a)
    for (size_t b = 0; b < a + 1; ++b)
      ...
      (*w_ccm).get(a, b) =
          2. * real(conj(get(psi0_cce).data()) * m.get(a) * m.get(b));
}
