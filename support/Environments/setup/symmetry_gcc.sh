...
. "${SPECTRE_HOME}/support/Environments/symmetry_gcc.sh"
...
if [ -f catch2/include/catch.hpp ]; then
    echo "Catch2 is already installed"
else
    echo "Installing catch..."
    # git clone git@github.com:catchorg/Catch2.git
    echo "Installed Catch into $dep_dir/catch2"
cat >$dep_dir/modules/catch2 <<EOF
#%Module1.0
prepend-path CPLUS_INCLUDE_PATH "$dep_dir/Catch2/include"
prepend-path CMAKE_PREFIX_PATH "$dep_dir/Catch2/"
EOF
fi
cd $dep_dir
