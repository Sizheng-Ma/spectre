...
// (most codes are copied from there). I construct a stationary Kerr spacetime
// in nontrivial time-dependent oscillating coordinates on both Cauchy and CCE
// grids. Then I compute psi0 on the worldtube. In principle, it should be
// consistent with 0.
...
void compute_psi0_of_bh_on_wt(const gsl::not_null<Generator*> gen) {
  ...
  // Now we need to transform the bondary data of BondiJ and BondiR from the
