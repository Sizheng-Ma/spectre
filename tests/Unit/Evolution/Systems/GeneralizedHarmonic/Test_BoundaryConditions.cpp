...
void test_constraint_preserving_bjorhus_u_minus_cce<
  ...
  MAKE_GENERATOR(gen);
  UniformCustomDistribution<double> w_distribution{-10000000000.0,
                                                   100000000000.0};
...
void test_constraint_preserving_bjorhus_u_minus<
  ...
  Scalar<SpinWeighted<ComplexDataVector, 2>> psi0_cce{
      number_of_angular_grid_points};
  for (size_t i = 0; i < get(psi0_cce).data().size(); ++i)
    get(psi0_cce).data()[i] = 0.;

  tnsr::a<ComplexDataVector, VolumeDim, frame> m_tetrad{slice_grid_points};
  GeneralizedHarmonic::AngularTetradForCCM<
      VolumeDim, Frame::Inertial>::get_m_vector(make_not_null(&m_tetrad),
                                                inertial_coords);
  ...
  GeneralizedHarmonic::InterpolatePsi0<VolumeDim, Frame::Inertial>::
      interpolate_psi0(make_not_null(&psi0_cce_inte), inertial_coords, psi0_cce,
                       l_max);

  tnsr::aa<DataVector, VolumeDim, frame> w_ccm{slice_grid_points};
  GeneralizedHarmonic::IncomingWFromCCE<VolumeDim, Frame::Inertial>::wijfromcce(
      make_not_null(&w_ccm), m_tetrad, psi0_cce_inte);
