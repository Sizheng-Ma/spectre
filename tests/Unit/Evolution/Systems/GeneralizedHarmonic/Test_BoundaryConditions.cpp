...
template <
    GeneralizedHarmonic::BoundaryConditions::Bjorhus::VMinusBcMethod BcMethod>
void test_constraint_preserving_bjorhus_u_minus_cce(
    size_t grid_size_each_dimension, const std::array<double, 3>& lower_bound,
    const std::array<double, 3>& upper_bound) noexcept;
  ...
  auto local_bc_dt_u_minus =
      GeneralizedHarmonic::Actions::BoundaryConditions_detail::set_dt_v_minus<
          ...
          VolumeDim>::apply(GeneralizedHarmonic::BoundaryConditions::Bjorhus::
                            ...
                            local_unit_normal_one_form, w_ccm);
...
void test_constraint_preserving_bjorhus_u_minus_cce<
  ...
  UniformCustomDistribution<double>
               w_distribution{-10000000000.0,100000000000.0};
  tnsr::aa<DataVector, VolumeDim, frame> w_ccm{slice_grid_points};
  tnsr::aa<DataVector, VolumeDim, frame> w_ccm_expect{slice_grid_points};
  for (size_t i = 0; i < slice_grid_points; ++i) {
      for (size_t a = 0; a <= VolumeDim; ++a) {
        for (size_t b = a; b <= VolumeDim; ++b) {
            w_ccm.get(a,b)[i] = w_distribution(gen);
            w_ccm_expect.get(a,b)[i] = 0.;
        }
      }
  }

  auto projection_AB = get<GeneralizedHarmonic::Actions::
               BoundaryConditions_detail::BjorhusIntermediatesComputer::
               internal_tags::projection_AB<VolumeDim, DataVector>>(buffer);
  auto projection_ab = get<GeneralizedHarmonic::Actions::
               BoundaryConditions_detail::BjorhusIntermediatesComputer::
               internal_tags::projection_ab<VolumeDim, DataVector>>(buffer);
  auto projection_Ab = get<GeneralizedHarmonic::Actions::
               BoundaryConditions_detail::BjorhusIntermediatesComputer::
               internal_tags::projection_Ab<VolumeDim, DataVector>>(buffer);
  auto char_speed = get<GeneralizedHarmonic::Actions::
               BoundaryConditions_detail::BjorhusIntermediatesComputer::
               internal_tags::char_speed_vminus<DataVector>>(buffer);

  for (size_t c = 0; c <= VolumeDim; ++c) {
    for (size_t d = c; d <= VolumeDim; ++d) {
      for (size_t a = 0; a <= VolumeDim; ++a) {
        for (size_t b = 0; b <= VolumeDim; ++b) {
          w_ccm_expect.get(c, d) += -(projection_Ab.get(a, c) *
               projection_Ab.get(b, d) - 0.5 * projection_ab.get(c, d) *
               projection_AB.get(a, b)) * get(char_speed) * w_ccm.get(a, b);
        }
      }
    }
  }
  ...
  for (size_t a = 0; a <= VolumeDim; ++a) {
    for (size_t b = a; b <= VolumeDim; ++b) {
      local_bc_dt_u_minus.get(a, b) -= w_ccm_expect.get(a, b);
    }
  }
...
void test_constraint_preserving_bjorhus_u_minus<
  ...
  Scalar<SpinWeighted<ComplexDataVector, 2>>
         psi0_cce{number_of_angular_grid_points};
  for(size_t i=0;i<get(psi0_cce).data().size();++i) get(psi0_cce).data()[i]=0.;

  tnsr::a<ComplexDataVector, VolumeDim, frame> m_tetrad{slice_grid_points};
  GeneralizedHarmonic::AngularTetradForCCM<VolumeDim,Frame::Inertial>
  ::get_m_vector(make_not_null(&m_tetrad),inertial_coords);

  Scalar<SpinWeighted<ComplexDataVector, 2>> psi0_cce_inte{slice_grid_points};
  GeneralizedHarmonic::InterpolatePsi0<VolumeDim,Frame::Inertial>::
  interpolate_psi0(make_not_null(&psi0_cce_inte), inertial_coords,
                   psi0_cce,l_max);

  tnsr::aa<DataVector, VolumeDim, frame> w_ccm{slice_grid_points};
  GeneralizedHarmonic::IncomingWFromCCE<VolumeDim,Frame::Inertial>::
  wijfromcce(make_not_null(&w_ccm),m_tetrad,psi0_cce_inte);
...
SPECTRE_TEST_CASE(
  ...
  test_constraint_preserving_bjorhus_u_minus_cce<
      GeneralizedHarmonic::BoundaryConditions::Bjorhus::VMinusBcMethod::
          ConstraintPreservingPhysical>(grid_size, lower_bound, upper_bound);
}
